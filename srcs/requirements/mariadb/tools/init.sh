...
# 	sleep 1
# 	mysql -e "CREATE DATABASE IF NOT EXISTS $DB_NAME CHARACTER SET utf8;CREATE USER IF NOT EXISTS '$MYSQL_USER'@'%' IDENTIFIED BY '$MYSQL_PASSWORD';GRANT ALL PRIVILEGES ON $DB_NAME.* TO '$MYSQL_USER'@'%' WITH GRANT OPTION;FLUSH PRIVILEGES;ALTER USER 'root'@'localhost' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD';DELETE FROM mysql.user WHERE User='';DROP DATABASE IF EXISTS test;DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%';FLUSH PRIVILEGES;"
# 	mysqladmin -uroot --password=$MYSQL_ROOT_PASSWORD shutdown
# fi
...
#     # Initialize the database and create users
#     mysql -h localhost -u root -p"$DB_ROOT_PW" <<EOF
# CREATE DATABASE IF NOT EXISTS $DB_NAME CHARACTER SET utf8;
# CREATE USER IF NOT EXISTS '$DB_USER'@'%' IDENTIFIED BY '$MARIADB_PW';
